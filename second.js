...
function sortDataA(json) {
    var result = [];
    function compareCost(firstCost, secondCost) {
        if (firstCost.price < secondCost.price)
            return 1;
        if (firstCost.price > secondCost.price)
            return -1;
        return 0;
    }
    function compareDate(firstTime, secondTime) {
        if (firstTime.fda_date_approved < secondTime.fda_date_approved)
            return 1;
        if (firstTime.fda_date_approved > secondTime.fda_date_approved)
            return -1;
        return 0;
    }
    function compareName(firstName, secondName) {
        if (firstName.company_name.toLowerCase() < secondName.company_name.toLowerCase())
            return -1;
        if (firstName.company_name.toLowerCase() > secondName.company_name.toLowerCase())
            return 1;
        return 0;
    }
    var nameLengths = json['Data example A'].map(function(cost) {
        if(cost.price){
            cost.price *= 1;
        }
        else{
        ...
        cost.fda_date_approved = new Date(cost.fda_date_approved);

        return cost
    });
    result['Data Example A: CostSorting'] = nameLengths.sort(compareCost);
    result['Data Example A: TimeSorting'] = nameLengths.sort(compareDate);
    result['Data Example A: NameSorting'] = nameLengths.sort(compareName);
    return result
}

function sortDataB(json) {
    var result = [];
    function compareName(firstName, secondName) {
        if (firstName.first_name.toLowerCase() < secondName.first_name.toLowerCase())
            return -1;
        if (firstName.first_name.toLowerCase() > secondName.first_name.toLowerCase())
            return 1;
        return 0;
    ...
    result['Data Example B: NameSorting'] = json['Data example B'].sort(compareName);
    return result
}

console.log(sortDataA(data));
console.log(sortDataB(data));